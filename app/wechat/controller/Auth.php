...
class Auth extends Frontend
{
    ...
    public function related()
    {
        ...
        // 将响应输出
        file_put_contents('../runtime/wechat.log',json_encode(input()));
        $response->send();
        exit();
    }
    /*
    ...
    {
        if(!file_exists('../runtime/wechat.log')) return false;
        $this->wxapp->server->push(function ($message) {
            if (!empty($message)) {
                $file = './wechatmsg.txt';
                if (!$file) {
                    @mkdir('./wechatmsg.txt', '777', true);
                }
                $files = fopen($file, 'a+');
